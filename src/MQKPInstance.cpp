...
void MQKPInstance::randomPermutation(int size, vector<int>& perm) {
	...
	perm.clear(); //Vaciamos el vector

	//Llenamos el vector con la permutacion identidad
	...
	//Mezclamos de manera aleatoria los valores del vector
	for(int i=0;i<size;i++){
		aux=perm[i];
		num=rand()%size;
		perm[i]=perm[num];
...
double MQKPInstance::getDeltaSumProfits(MQKPSolution& solution, int indexObject,
	 ...
	 */
	if(solution.whereIsObject(indexObject)!=0){ //El objeto esta en una mochila
		for(int i=0;i<getNumObjs();i++){ //Exploramos todos los objetos
			if(solution.whereIsObject(indexObject)==solution.whereIsObject(i)){ //Ambos objetos estan en la misma mochila
				deltaSumProfits-=getProfit(indexObject,i);
			}
		}
	}
	 ...
	 */
	//else{ //El objeto no esta en ninguna mochila
	if(indexKnapsack!=0){
		for(int i=0;i<getNumObjs();i++){ //Exploramos todos los objetos
			if(solution.whereIsObject(i)==indexKnapsack){ //El objeto explorado esta en la mochila recibida como argumento
				deltaSumProfits+=getProfit(indexObject,i);
			}
		}
	}
...
double MQKPInstance::getDeltaMaxCapacityViolation(MQKPSolution& solution,
	 ...
	 */
	int auxIndexKnapsack;
	double aux;

	auxIndexKnapsack=solution.whereIsObject(indexObject); //Averiguamos en que mochila esta el objeto
	aux=getMaxCapacityViolation(solution); //Hallamos la violacion maxima actual
	solution.putObjectIn(indexObject,indexKnapsack); //Colocamos el objeto en la nueva mochila
	aux=getMaxCapacityViolation(solution)-aux; //Hallamos la diferencia entre la nueva violacion maxima y la actual
	solution.putObjectIn(indexObject,auxIndexKnapsack); //Metemos el objeto en la mochila en la que estaba
